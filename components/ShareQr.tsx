...
export default function ShareQr({
  ...
  useEffect(() => {
    if (!stripDataUrl || !videoBlob) return;
    ...
    (async () => {
      try {
        ...
        // 영상: 브라우저에서 Vercel Blob으로 직접 업로드 (서버 함수의
        // 요청 용량 제한을 우회해서 고화질 그대로 올릴 수 있다)
        const videoExt = videoBlob.type.includes("mp4") ? "mp4" : "webm";
        const videoUploadPromise = upload(
          `strips/session-${Date.now()}.${videoExt}`,
          videoBlob,
          {
            access: "public",
            handleUploadUrl: "/api/share/video-token",
            contentType: videoBlob.type || `video/${videoExt}`,
          },
        );
        ...
        )}&video=${encodeURIComponent(videoResult.url)}`;
